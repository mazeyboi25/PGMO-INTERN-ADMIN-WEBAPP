...
alter table public.registration_invites alter column expires_at set default '2099-12-31 23:59:59+00'::timestamptz; -- remove this when turning on expiry
-- alter table public.registration_invites alter column expires_at set default now() + interval '1 hour';
update public.registration_invites
set expires_at = '2099-12-31 23:59:59+00'::timestamptz
where status = 'unused'; -- remove this when turning on expiry
-- update public.registration_invites
-- set max_attempts = 3
...
begin
    ...
    insert into public.registration_invites (
        ...
        0,
        3,
        '2099-12-31 23:59:59+00'::timestamptz, --remove this when turning on expiry
        -- now() + interval '1 hour', -- remove comment when turning on expiry
        null,
        now()
    );
...
as $$
    select
        ...
        ri.office_assigned,
        ri.contact_number,
        ri.status as status, --remove this when turning on expiry
        -- case
           -- when ri.status = 'unused' and ri.expires_at < now() then 'expired'
           -- else ri.status
        -- end as status, -- rmeove comment when turning on
        ri.attempts,
        ri.max_attempts,
        ri.expires_at,
        ri.used_at,
